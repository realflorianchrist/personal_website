...
export default function WallShelf(props: Readonly<ThreeElements["group"]>) {
  return (
    <group {...props} dispose={null}>
      <Shelf scale={[1.3, 1, 1]} position={[0, -0.07, 0]} />
      <Spiderman position={[0.5, 0, 0]} />
      <Ezio
        rotation={[0, Math.PI / 3 + Math.PI, 0]}
        position={[0.15, -0.022, 0.05]}
      />
      <Itachi position={[-0.1, 0, 0]} />
      <Obito position={[-0.35, 0, 0]} />
    </group>
  );
}
