  ...
  return (
    <main>
      ...
      <ul className={`animate dur800 slideInBottom flex-col content-center pt-10 mb-14`}>
        {i18n.About.Timeline.map((waypoint: Waypoint, index: number) => {
          ...
          return (
            <li key={waypoint.description}
                className={`animate ${durationClass} slideInBottom ${liClass} block relative mb-8 md:mb-12`}>
              <div className={"h-3 w-3 rounded-full bg-accent-color absolute top-0.5 left-1/2 transform -translate-x-1/2"}></div>
        ...
        <div className={"absolute top-0 left-1/2 w-1 h-full bg-accent-color transform -translate-x-1/2 rounded-full"}></div>
      </ul>
    </main>
  );
}